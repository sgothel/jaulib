...
static constexpr std::string format_string_static3(const std::string_view format, const Targs &...args) {
    ...
    if( 0 <= jau::cfmt2::impl::checkRec<Targs...>( jau::cfmt2::PResult(format)).argCount() ) {
        return jau::format_string_n(1024, format, args...);
    } else {
        return "";
    }
}
...
TEST_CASE("jau::cfmt_00", "[jau][std::string][jau::cfmt]") {
    ...
    {   // 'format_check: %.2f, %2.2f, %zu, %lu, %03d'
        constexpr  jau::cfmt2::PResult pc = jau::cfmt2::checkR("format_check: %.2f, %2.2f, %zu, %" PRIi64 ", %" PRIi64 "d, %" PRIi64 "X, %06" PRIu64 "d, %06" PRIu64 "X, %03d\n",
                                                               fa, fb, sz1, v_i64, v_i64, v_i64, v_u64, v_u64, i);
        std::cerr << "XXX: " << __LINE__ << ": " << pc << std::endl;
        // static_assert( 5 == pc.argCount() );
        REQUIRE(9 == pc.argCount());
    }
    {
    ...
    }
    {
        ...
        static_assert(true == jau::cfmt::check("    uint64_t -> int64_t %" PRIi64, (uint64_t)1));
        static_assert(true == jau::cfmt::check("     int64_t -> uint64_t %" PRIu64, (int64_t)1));
        static_assert(true == jau::cfmt::check("        char -> int %d", (char)1)); // integral target type > given type
        {
            jau::cfmt::PResult res = jau::cfmt::checkR(" error long -> int %d", (long)1);
            // if( res.error() ) {
            printf("XXX: sizeof(long) %zu, %s\n", sizeof(long), res.toString().c_str());
        }
        if constexpr ( sizeof(long) <= 4 ) {
            assert(true == jau::cfmt::check(" OK long(4) -> int %d", (long)1)); // NOLINT(misc-static-assert)
        } else {
            assert(false == jau::cfmt::check(" error long(8) -> int %d", (long)1)); // NOLINT(misc-static-assert); error: integral target type < given type
        }
        ...
        static_assert(true == jau::cfmt::check(" %" PRIi64 ", %" PRIi64 ", %08" PRIi64 ".", (int64_t)1, (int64_t)1, (int64_t)1));
        static_assert(true == jau::cfmt::check(" %" PRIi64 ", %" PRIi64 ", %08" PRIi64 ".", v_i64, v_i64, v_i64));
        static_assert(true == jau::cfmt::check(" %p", pf));
        // static_assert(true == jau::cfmt::check(" %s", "lala"));
        ...
        static_assert(5 == jau::cfmt::checkR("Hello %" PRIu64 ", %" PRIu64 ", %" PRIx64 ", %06" PRIu64 ", %06" PRIx64 "",
                                             v_i64, v_i64, v_i64, v_i64, v_i64).argCount());
...
TEST_CASE("jau::cfmt_10", "[benchmark][jau][std::string][format_string]") {
    ...
    BENCHMARK("fmt__checkR2          bench") {
        constexpr static std::string_view format = "format_check: %.2f, %2.2f, %zu, %" PRIu64 ", %03d\n";
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            size_t r = jau::cfmt::checkR2<float, float, size_t, uint64_t, int>(format).argCount();
            REQUIRE(5 == r);
            res = res + r;
        }
        return res;
    };
    BENCHMARK("fmt__checkR2 cnstexpr bench") {
        constexpr static std::string_view format = "format_check: %.2f, %2.2f, %zu, %" PRIu64 ", %03d\n";
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            constexpr size_t r = jau::cfmt::checkR2<float, float, size_t, uint64_t, int>(format).argCount();
            REQUIRE(5 == r);
            res = res + r;
        }
        return res;
    ...
    BENCHMARK("format_010a_jaufmtstr bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            res = res + test_format(format_010a_jaufmtstr, false);
        }
        return res;
    };
    BENCHMARK("format_020a jau macro bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            {
                constexpr float fa = 1.1f, fb = 2.2f;
    ...
    BENCHMARK("fmt__020a_jaufmtstr_n bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            res = res + test_format(format_020a_jaufmtstr_n, false);
        }
        return res;
    };
    ...
    BENCHMARK("format_000b_vsnprintf__ bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
    ...
    BENCHMARK("format_010b_jaufmtstr__ bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            res = res + test_format(format_010b_jaufmtstr, false);
        }
        return res;
    };
    BENCHMARK("format_020b_jaufmtstr_n bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
            res = res + test_format(format_020b_jaufmtstr_n, false);
        }
        return res;
    };
    BENCHMARK("format_030b_strstream__ bench") {
        volatile size_t res = 0;
        for( size_t i = 0; i < loops; ++i ) {
