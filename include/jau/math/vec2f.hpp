...
#ifndef JAU_MATH_VEC2F_HPP_
#define JAU_MATH_VEC2F_HPP_
...
namespace jau::math {
    ...
    class alignas(sizeof(Value_type)) Vector2F : public VectorNT<Value_type, Vector2F<Value_type>, 2>
    {
      public:
        using VectorBase = VectorNT<Value_type, Vector2F<Value_type>, 2>;
        ...
        : VectorBase(v[0]), y(v[1]) {}

        constexpr Vector2F(std::initializer_list<value_type> v) noexcept { VectorBase::set(v.begin(), v.end()); }

        template<typename container_type>
        requires jau::req::contiguous_container<container_type>
        constexpr Vector2F(const container_type &c) noexcept { VectorBase::set(c.cbegin(), c.cend()); }
    ...
    template<jau::req::packed_floating_point T>
    constexpr Vector2F<T> min(const Vector2F<T>& lhs, const T rhs) noexcept {
        return Vector2F<T>(std::min(lhs.x, rhs), std::min(lhs.y, rhs));
    }
    ...
    template<jau::req::packed_floating_point T>
    constexpr Vector2F<T> max(const Vector2F<T>& lhs, const T rhs) noexcept {
        return Vector2F<T>(std::max(lhs.x, rhs), std::max(lhs.y, rhs));
    }

    /// Returns the component-wise clamped-value of given value-vector to the min_val and max_val vector
    template<jau::req::packed_floating_point T>
    constexpr Vector2F<T> clamp(const Vector2F<T>& v, const Vector2F<T>& min_val, const Vector2F<T>& max_val) noexcept {
        return Vector2F<T>(std::min(std::max(v.x, min_val.x), max_val.x),
    ...
    template<jau::req::packed_floating_point T>
    constexpr Vector2F<T> clamp(const Vector2F<T>& v, const T min_val, const T max_val) noexcept {
        return Vector2F<T>(std::min(std::max(v.x, min_val), max_val),
    ...
    static_assert(true == jau::req::contiguous_container<Point2f>);
